...
class RectDetect:
    ...
    def _ensure_remap(self, w: int, h: int) -> None:
        ...
        self._mapx, self._mapy = cv2.initUndistortRectifyMap(
            self._mtx, self._dist, None, new_mtx, size, cv2.CV_32FC1,
        )
    ...
    def _undist_to_dist(self, pts_undist: np.ndarray) -> np.ndarray:
        ...
        fx, fy = self._mtx[0, 0], self._mtx[1, 1]
        cx, cy = self._mtx[0, 2], self._mtx[1, 2]
    ...
    def _solve_pnp(
        ...
        ok, rvec, tvec = cv2.solvePnP(
            self._obj_points, pts, self._mtx, None,
        ...
        reproj, _ = cv2.projectPoints(
            self._obj_points, rvec, tvec, self._mtx, None,
        )
    ...
    def _search(
        ...
        for area, cnt in scored:
            ...
            _, err, depth, ox, oy = self._solve_pnp(ordered)

            self._last_err = err
            self.pass_rate = 1.0 if err < PNP_MAX_REPROJ_ERROR else 0.0
    ...
    def detect(
        ...
        self._gray_undist = cv2.remap(gray, self._mapx, self._mapy, cv2.INTER_LINEAR)
        ...
        # ---- 2. ROI 搜索 ----
        if self.last_rect is not None and self.lost_count < MAX_TRACK_LOST:
            rx, ry, rw, rh = self._expand_roi(self.last_rect, w, h)
